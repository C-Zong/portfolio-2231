...
I chose `Stack2` to represent the budget tracker component. It's based on a linked list, so it's easier to add or remove elements. Since it's a stack, and in transactions we often care more about the most recent activity, the Last-In-First-Out (LIFO) behavior is a good fit.
